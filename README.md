...
학습 데이터는 LogicKor 문항별로 가장 높은 Judge 점수를 받은 모델 답변을 few-shot 예시로 사용해 증강했습니다.
...
증강 시에는 few-shot 문장 복사를 금지하고, 원본 LogicKor의 category, 난이도, 2-turn 대화 구조를 유지하도록 제한했습니다.

### 학습 전략

공유 모델은 Qwen3-8B를 기반으로 `logickor_sft_high_converted.jsonl` 데이터에 대해 LoRA SFT를 수행한 모델입니다.
